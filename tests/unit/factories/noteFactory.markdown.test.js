...
//
// OBSOLETE: These tests were designed for the old architecture where noteFactory
// handled markdown rendering directly. With the new unified architecture:
// - noteFactory creates pure DOM elements
// - DesktopAdapter/TouchAdapter handle events
// - EditModeController manages edit/view mode transitions
// - editViewMode.js handles markdown rendering
//
// These integration behaviors should be tested at the E2E level, not unit level.
...
describe('Note Factory with Markdown Integration [OBSOLETE - Wrong Architecture Level]', () => {
  let canvas;
  ...
  test.skip('OBSOLETE: should render markdown on blur and show raw text on focus', async () => {
  ...
  test.skip('OBSOLETE: should handle simple markdown rendering', async () => {
    const note = createNote(100, 100, canvas);
    const noteContent = note.querySelector('.note-content');
  ...
  test.skip('OBSOLETE: should preserve line breaks in markdown', async () => {
    const note = createNote(100, 100, canvas);
    const noteContent = note.querySelector('.note-content');
