...
describe('Refresh Bug Diagnosis', () => {
  ...
  it('should reproduce the exact steps from manual testing', () => {
    ...
    const notes = [{ id: 'test-note', content: originalMarkdown, left: 100, top: 100 }];
    ...
    if (corruptedContent && corruptedContent.includes('<')) {
      ...
      const corruptedNotes = [{ id: 'test-note', content: corruptedContent, left: 100, top: 100 }];
      ...
      console.log('If innerHTML (HTML) accidentally gets saved instead of dataset.markdown,');
      console.log('the canonical storage will defang it to plain text.');
      console.log(`Original: "${originalMarkdown}" -> Corrupted: "${corruptedContent}" -> Defanged: "${reloaded.notes[0].content}"`);
    }
  });
  ...
  it('should test if the bug could be in note content extraction', () => {
    ...
    const brokenNote = {
      ...
      dataset: { markdown: '# Header' },
      setAttribute: function(attr, value) {},
      getAttribute: function(attr) { return null; }, // Always returns null
      classList: { contains: () => false },
    };
