...
test.describe('Touch Connection Context Menu', () => {
  ...
  test('should not show context menu when tapping on connection in desktop mode', async ({
    page,
  }) => {
    const canvasPage = new CanvasPage(page);

    // Load in desktop mode (no touch mode parameter)
    ...
    // Try clicking on connection (should not show context menu in desktop mode)
    const connectionPath = connection.locator('path');
    await connectionPath.first().click();

    // Wait a moment to see if context menu appears
    await page.waitForTimeout(500);

    // Context menu should NOT be visible after click in desktop mode
    const contextMenu = page.locator('.context-menu');
