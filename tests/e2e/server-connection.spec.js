...
test.describe('Server Connection UI (MM-104)', () => {
  ...
  test('should show server connection modal when menu item is clicked', async ({ page }) => {
    // Open kebab menu
    ...
    await expect(page.locator('#server-modal-title')).toHaveText('Connect to Server');
  ...
  test('should validate server URI input', async ({ page }) => {
    ...
    await expect(page.locator('#server-uri-input')).toHaveValue('https://api.example.com');
  });
  ...
  test('should NOT close modal when clicking outside (better UX)', async ({ page }) => {
    // Open server connection modal
    await page.click('#kebab-menu-button');
    await page.click('[data-action="connect-server"]');
    await expect(page.locator('#server-connection-modal')).toBeVisible();
  ...
  test('should show disconnect option when connected (placeholder)', async ({ page }) => {
  ...
  test('should maintain form state when modal is reopened', async ({ page }) => {
    // Open server connection modal
    await page.click('#kebab-menu-button');
    await page.click('[data-action="connect-server"]');
    await expect(page.locator('#server-connection-modal')).toBeVisible();
  ...
  test('should allow typing and persist URL input without losing focus', async ({ page }) => {
    // Open server connection modal
    await page.click('#kebab-menu-button');
    await page.click('[data-action="connect-server"]');
    await expect(page.locator('#server-connection-modal')).toBeVisible();
  ...
  test('should maintain input data during form interactions', async ({ page }) => {
    // Open server connection modal
    await page.click('#kebab-menu-button');
    await page.click('[data-action="connect-server"]');
    await expect(page.locator('#server-connection-modal')).toBeVisible();
