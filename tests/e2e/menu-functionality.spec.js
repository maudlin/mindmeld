...
test.describe('Menu Functionality', () => {
  test.describe('Navigation Menu Structure', () => {
    test('Should display all main menu items', async ({ page }) => {
      ...
      await expect(page.locator('button:has-text("Canvas Style")')).toBeVisible();
    ...
    test('Should show About dropdown menu on hover', async ({ page }) => {
      ...
      await expect(page.locator('a[href="about.html"]:has-text("About MindMeld")')).toBeVisible();
      await expect(page.locator('a[href="changelog.html"]:has-text("What\'s New")')).toBeVisible();
    });

    test('Should show Import/Export dropdown menu on hover', async ({ page }) => {
      const canvasPage = new CanvasPage(page);
      await canvasPage.load();
  ...
  test.describe('Clear Canvas Functionality', () => {
    test('Should clear all notes and connections when confirmed', async ({ page }) => {
      const canvasPage = new CanvasPage(page);
      await canvasPage.load();
      ...
      page.on('dialog', async dialog => {
        expect(dialog.message()).toContain('Are you sure you want to clear the canvas?');
        await dialog.accept();
      });
      ...
      const connectionGroups = canvasPage.svgContainer.locator('g[data-start][data-end]');
      await expect(connectionGroups).toHaveCount(0);
    });

    test('Should preserve content when clear is cancelled', async ({ page }) => {
      const canvasPage = new CanvasPage(page);
      await canvasPage.load();
  ...
  test.describe('Import/Export Functionality', () => {
    test('Should export to clipboard button be accessible and functional', async ({ page }) => {
      const canvasPage = new CanvasPage(page);
      await canvasPage.load();
      ...
      page.on('dialog', async dialog => {
        ...
        expect(dialog.message()).toMatch(/Mind map exported to clipboard!|Failed to copy to clipboard/);
        await dialog.accept();
      });
    ...
    test('Should import from clipboard button be accessible', async ({ page }) => {
      const canvasPage = new CanvasPage(page);
      await canvasPage.load();
      ...
      page.on('dialog', async dialog => {
        ...
        expect(dialog.message()).toMatch(/imported from clipboard|Error importing from clipboard|Failed to read from clipboard/);
        await dialog.accept();
      });
    ...
    test('Should handle import from file', async ({ page }) => {
      ...
      // Create test JSON data for import
      const testData = {
        data: {
          n: [
            { i: "1", p: [400, 300], c: "Imported Note 1" },
            { i: "2", p: [700, 300], c: "Imported Note 2" }
          ],
          c: [["1", "2", 1]]
        }
      };
    ...
    test('Should handle invalid clipboard data gracefully', async ({ page }) => {
      const canvasPage = new CanvasPage(page);
      await canvasPage.load();
    ...
    test('Should test export/import workflow via file operations', async ({ page }) => {
      const canvasPage = new CanvasPage(page);
      await canvasPage.load();
