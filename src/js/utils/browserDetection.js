...
export function detectBrowser() {
  ...
  const isChrome = /Chrome/.test(userAgent) && (/Google Inc/.test(vendor) || vendor === '' || /Chromium/.test(userAgent));
...
export function getStorageLimitations(browserInfo, isPrivateMode = false) {
  ...
  if (browserInfo.isSafari && isPrivateMode) {
    return {
      ...
      warning: 'Safari Private Browsing has very limited storage. Consider using normal browsing mode for better performance.',
...
export function getStorageErrorMessage(error, browserInfo, isPrivateMode) {
  ...
  // Safari private mode
  if (browserInfo.isSafari && (isPrivateMode || errorMessage.includes('private') || errorMessage.includes('disabled'))) {
  ...
  if ((browserInfo.isChrome || browserInfo.isEdge) && errorMessage.toLowerCase().includes('quota')) {
    const browserName = browserInfo.isEdge ? 'Edge' : 'Chrome';
