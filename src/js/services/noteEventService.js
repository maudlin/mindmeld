...
export class NoteEventService {
  static initialize() {
    ...
    eventBus.on('note.createAtPosition', ({ canvas, event }) => {
      console.log('NoteEventService: Received note.createAtPosition event', { canvas: canvas?.id, event: event?.type });
    ...
    eventBus.on('note.deleteWithConnections', ({ note, canvas }) => {
      console.log('NoteEventService: Received note.deleteWithConnections event');
